...
const CANDIDATES = [
  ...
  },
  {
    ...
    experience: [
      { title: "平面設計總監", company: "磐石電腦", period: "2020-2025", duties: ["領導硬體產品標誌設計與包裝開發", "累積 10 項設計專利與 5 項設計獎"] },
...
【語氣指令】
請以肯定、平鋪直敘的語氣回覆，使用明確、直接的表達方式。避免使用任何模糊語或不確定詞彙（例如「可能」、「也許」、「我猜」、「不太確定」、「應該是」等）。所有判斷皆以肯定句呈現。

【範圍限制】
若使用者詢問與履歷或面試無關的問題，請以一兩句簡短回應該問題後，主動引導使用者回到對該候選人的討論。例如可說：「關於您的問題……（簡短回應）。不過讓我們回到陳O恩的履歷，您想了解哪個部分呢？」
...
【語氣指令】
請以肯定、平鋪直敘的語氣回覆，使用明確、直接的表達方式。避免使用任何模糊語或不確定詞彙。所有判斷皆以肯定句呈現。

【範圍限制】
若使用者詢問與履歷或面試無關的問題，請以一兩句簡短回應該問題後，主動引導使用者回到對該候選人的討論。例如可說：「關於您的問題……（簡短回應）。不過讓我們回到林O的履歷，您想了解哪個部分呢？」
...
【範圍限制】
若使用者詢問與履歷或面試無關的問題，請以一兩句簡短回應該問題後，主動引導使用者回到對該候選人的討論。例如可說：「關於您的問題……（簡短回應）。不過讓我們回到王O穎的履歷，您想了解哪個部分呢？」
...
學歷：工館大學 傳播學程（大學）
經歷：
- 平面設計總監，磐石電腦，2020-2025，領導硬體產品標誌設計與包裝開發、累積 10 項設計專利與 5 項設計獎
...
【範圍限制】
若使用者詢問與履歷或面試無關的問題，請以一兩句簡短回應該問題後，主動引導使用者回到對該候選人的討論。例如可說：「關於您的問題……（簡短回應）。不過讓我們回到張O安的履歷，您想了解哪個部分呢？」
...
app.post('/api/start_candidate', async (req, res) => {
  ...
  const systemInstruction = SYSTEM_PROMPTS.a[candidateIndex] ?? `You are an HR Assistant helping evaluate candidate ${candidate.name}.`;

  let chatSession = null;
  const aiClient = aiClients.a;
