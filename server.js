...
【輸出格式】
請以純文字回覆，不使用 markdown 格式。每次回覆控制在 80 至 120 字內。`,
...
app.post('/api/start_candidate', async (req, res) => {
  ...
  const branchPrompts = SYSTEM_PROMPTS[branch.toLowerCase()];
  const systemInstruction = branchPrompts?.[candidateIndex] ?? `You are an HR Assistant helping evaluate candidate ${candidate.name}.`;
